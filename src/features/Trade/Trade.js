...
const Trade = ({ players, playerId, roomId, resetStates }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        players[0].map((player) => {
            if (player.player_id === playerId && player.property_value >= amountToTradeAway) {
                ...
                players[0].map((player) => {
                    if (player.player_id === playerToTradeWith.player_id && playerToTradeWith.property_value >= amountToTradeWith) {
                        player.property_value -= amountToTradeWith
                        player.net_worth -= amountToTradeWith
                        player.property_value += amountToTradeAway
                        player.net_worth += amountToTradeAway
                        ...
                        return (
                            <div className='trade-div' key={player.player_id}>
                                <p>Select Player to Trade With:</p>
                                    ...
                                    {playerToTradeWith && playerToTradeWith.property_value <= 0 ? (
                                        <p>Invalid! {playerToTradeWith.name} does not have anything to trade!</p>
                                    ) : (
                                        playerToTradeWith !== '' ? (
                                            <form key={player.player_id} onSubmit={handleSubmit}>
                                                ...
                                                <input type='number' value={amountToTradeWith} name='player-to-trade-with-prop-value' onChange={(e) => setAmountToTradeWith(Math.round(e.target.value))}/>
                                                {amountToTradeWith > playerToTradeWith.property_value ? (
                                                    <p>Invalid! Insufficient Property Value!</p>
                                                ) : (
                                                        amountToTradeWith === '' ? '' : (
                                                            <div>
                                                                <label htmlFor='amount-to-trade-away'>Enter Your Property Value:</label>
                                                                <input type='amount' value={amountToTradeAway} name='amount-to-trade-away' onChange={(e) => setAmountToTradeAway(Math.round(e.target.value))}/>
